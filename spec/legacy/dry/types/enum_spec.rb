RSpec.describe Legacy::Dry::Types::Enum do
  context 'with mapping' do
    ...
    let(:string) { Legacy::Dry::Types['strict.string'] }
  ...
  context 'with int type' do
    subject(:type) { Legacy::Dry::Types['integer'].enum(*values) }
  ...
  describe '#include?' do
    subject(:enum) { Legacy::Dry::Types['integer'].enum(4, 5, 6) }
  ...
  describe '#with' do
    subject(:enum_with_meta) { Legacy::Dry::Types['integer'].enum(4, 5, 6).with(meta: { foo: :bar }) }
