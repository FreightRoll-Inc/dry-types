RSpec.describe Legacy::Dry::Types::Definition, '#safe' do
  context 'with a coercible string' do
    subject(:type) { Legacy::Dry::Types['coercible.string'].constrained(min_size: 5).safe }
  ...
  context 'with a params hash' do
    subject(:type) do
      Legacy::Dry::Types['params.hash'].weak(age: 'coercible.integer', active: 'params.bool')
    end
