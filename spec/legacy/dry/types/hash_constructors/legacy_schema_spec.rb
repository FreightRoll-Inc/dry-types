RSpec.describe Legacy::Dry::Types::Hash do
  subject(:hash) do
    Legacy::Dry::Types['hash'].schema(hash_schema)
  end
  ...
  let(:hash_schema) do
    {
      ...
      age: "strict.integer",
      active: "params.bool",
      phone: Legacy::Dry::Types['phone']
    }
  end

  let(:phone) { Legacy::Dry::Types['phone'].primitive }
  ...
  before do
    ...
    Legacy::Dry::Types.register(
      "phone",
      Legacy::Dry::Types::Definition.new(phone).constructor(-> args { phone.new(*args) })
    )
  end

  shared_examples 'hash schema behavior' do
    let(:type) { Legacy::Dry::Types['hash'] }
    ...
    it 'yields failure on #try when applying a member type fails' do
      ...
      expect { |rspec_probe| hash.try(input, &rspec_probe) }
        .to yield_with_args(instance_of(Legacy::Dry::Types::Result::Failure))
  ...
  shared_examples 'strict schema behavior for missing keys' do
    it 'raises MissingKeyError if input is missing a key' do
      expect {
        ...
        Legacy::Dry::Types::MissingKeyError, /:age is missing in Hash input/
      )
    end
  end
  ...
  shared_examples 'strict typing behavior' do
    it 'fails if any coercions are unsuccessful' do
      expect { hash.call(name: :Jane, age: 'oops', active: true, phone: []) }
        .to raise_error(Legacy::Dry::Types::SchemaError, '"oops" (String) has invalid type for :age violates constraints (type?(Integer, "oops") failed)')
    end
  end
  ...
  shared_examples 'sets default value behavior when keys are omitted' do
    context 'when default value for :age is 21' do
      let(:hash_schema) do
        {
          name: "coercible.string",
          age: Legacy::Dry::Types["strict.integer"].default(21),
          active: "params.bool",
          phone: Legacy::Dry::Types['phone']
        }
      end
  ...
  shared_examples 'strict schema behavior for unexpected keys' do
    it 'rejects unexpected keys' do
      ...
      expect { hash.call(expected_input.merge(unexpected_input)) }
        .to raise_error(Legacy::Dry::Types::UnknownKeysError)
    ...
    context 'when default value for :age is 21' do
      ...
      it 'raises an error when nil passed to a default' do
        expect { hash.call(name: :John, active: '1', age: nil, phone: []) }
          .to raise_error(Legacy::Dry::Types::SchemaError, 'nil (NilClass) has invalid type for :age violates constraints (type?(Integer, nil) failed)')
      end
    end
  end

  describe '#schema' do
    let(:hash) { primitive.schema(hash_schema, :schema) }
  ...
  describe '#strict' do
    ...
    context 'with a sum' do
      ...
      let(:h1) { Legacy::Dry::Types['strict.hash'].strict(name: Legacy::Dry::Types['strict.string']) }
      let(:h2) { Legacy::Dry::Types['strict.hash'].strict(age: Legacy::Dry::Types['strict.integer']) }
      ...
      it 'raises error when it is invalid for both sides' do
        expect { hash[oops: 'boom!'] }.to raise_error(Legacy::Dry::Types::ConstraintError, /oops/)
      end
    end
  end
