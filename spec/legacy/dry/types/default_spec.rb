RSpec.describe Legacy::Dry::Types::Definition, '#default' do
  context 'with a definition' do
    subject(:type) { Legacy::Dry::Types['string'].default('foo') }
  ...
  context 'with a constrained type' do
    it 'does not allow a value that is not valid' do
      expect {
        Legacy::Dry::Types['strict.string'].default(123)
      }.to raise_error(
        Legacy::Dry::Types::ConstraintError, /123/
      )
    end
  end
  ...
  context 'with meta attributes' do
    context 'default called first' do
      subject(:type) { Legacy::Dry::Types['hash'].default({}).meta(omittable: true) }
    ...
    context 'default called last' do
      subject(:type) { Legacy::Dry::Types['hash'].meta(omittable: true).default({}) }
  ...
  context 'with an optional type' do
    subject(:type) { Legacy::Dry::Types['strict.integer'].optional.default(nil) }
  ...
  context 'with a strict bool' do
    ...
    it_behaves_like 'Legacy::Dry::Types::Definition without primitive' do
      let(:type) { Legacy::Dry::Types['strict.bool'].default(false) }
    end
  ...
  context 'with a callable value' do
     context 'with 0-arity block' do
      subject(:type) { Legacy::Dry::Types['time'].default { Time.now } }
     ...
     context 'with 1-arg block' do
      ...
      subject(:type) do
        Legacy::Dry::Types['time'].constructor(&floor_to_date).default { |type| type[Time.now] }
      end
  ...
  describe 'decorator' do
    subject(:type) { Legacy::Dry::Types['strict.string'].default('foo') }
  ...
  describe'#with' do
    subject(:type) { Legacy::Dry::Types['time'].default { Time.now }.with(meta: { foo: :bar }) }
  ...
  it 'works with coercible.array' do
    base = Legacy::Dry::Types['coercible.array'].default([].freeze)
    type = base.of(Legacy::Dry::Types['string'])
